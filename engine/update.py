...
def _dashboard_json_path(name):
    return f"{config.DASHBOARD_JSON_PREFIX}/{name}.json"
...
def _df_records(df):
    ...
    for col in clean.columns:
        if pd.api.types.is_datetime64_any_dtype(clean[col]):
            clean[col] = clean[col].dt.strftime("%Y-%m-%dT%H:%M:%SZ")
    return json.loads(clean.to_json(orient="records"))


def _read_repo_json(path):
    with _resolve_repo_path(path).open(encoding="utf-8") as f:
        return json.load(f)


def _bucket_rows_from_latest(latest, buckets):
    total_wallets = int(sum(latest[count_col] for _, count_col, _ in buckets))
    rows = []
    for pct_col, count_col, label in buckets:
        wallets = int(latest[count_col])
        rows.append({
            "bucket": label,
            "wallets": wallets,
            "walletSharePct": (wallets / total_wallets * 100) if total_wallets else 0.0,
            "supplySharePct": float(latest[pct_col]),
        })
    return total_wallets, rows


def _build_coin_age_example_bundle():
    example_path = _resolve_repo_path("examples/coin_age_example_wallet_events.csv")
    events = pd.read_csv(example_path)
    events["timestamp"] = pd.to_datetime(events["timestamp"], utc=True)
    events["log_index"] = events["event_id"].str.rsplit(":", n=1).str[-1].astype(int)
    events = events.sort_values(["block_number", "log_index"]).reset_index(drop=True)

    as_of = pd.Timestamp("2026-05-25T00:00:00Z")
    balance = 0.0
    age_mass = 0.0
    current = events["timestamp"].iloc[0] if not events.empty else as_of
    rows = []
    total_in = 0.0
    total_out = 0.0
    peak_balance = 0.0
    ...
    for _, row in events.iterrows():
        ...
        avg_age_before = age_mass / balance if balance > 0 else 0.0
        ...
        rows.append({
            ...
            "ageBefore": avg_age_before,
            "balanceAfter": balance,
            "ageAfter": age_mass / balance if balance > 0 else 0.0,
        })
        current = ts

    if balance > 0 and current < as_of:
        age_mass += balance * ((as_of - current).total_seconds() / 86400)

    return {
        "wallet": "0xffb3f0b6817036985f49c311a2f7d597bcb02910",
        "asOf": as_of.strftime("%Y-%m-%dT%H:%M:%SZ"),
        "summary": {
            "currentBalance": balance,
            "peakBalance": peak_balance,
            "ofPeak": balance / peak_balance if peak_balance else 0.0,
            "totalIn": total_in,
            "totalOut": total_out,
            "soldBought": total_out / total_in if total_in else 0.0,
            "avgCoinAgeDays": age_mass / balance if balance > 0 else 0.0,
        },
        "rows": rows,
    }


def build_dashboard_bundles():
    """
    Build private, app-ready JSON bundles for the Vercel dashboard.
    ...
    bucket_breakdown["date"] = pd.to_datetime(bucket_breakdown["date"])
    holder_growth["date"] = pd.to_datetime(holder_growth["date"])
    chad_cohorts["date"] = pd.to_datetime(chad_cohorts["date"])
    soulbound_supply["date"] = pd.to_datetime(soulbound_supply["date"])
    ...
    latest_bucket = bucket_breakdown.loc[bucket_breakdown["date"].idxmax()]
    total_wallets, wallet_vs_supply = _bucket_rows_from_latest(latest_bucket, buckets)

    latest_chads = chad_cohorts.sort_values("date").groupby("cohort", observed=False).tail(1)
    total_chad_balance = float(latest_chads["total_balance"].sum()) if not latest_chads.empty else 0.0
    weighted_age = (
    ...
    holders_path = _resolve_repo_path(config.SOULBOUND_NFT_HOLDERS_CSV)
    soulbound_holders = pd.read_csv(holders_path)
    soulbound_holders["address"] = soulbound_holders["HolderAddress"].astype(str).str.lower()
    soulbound_holders = soulbound_holders.rename(columns={"Quantity": "nft_quantity"})
    ...
    soulbound_wallets = soulbound_holders.merge(
        snapshot[[address_col, "balance"]],
        left_on="address",
        right_on=address_col,
        how="left",
    )
    soulbound_wallets["balance"] = soulbound_wallets["balance"].fillna(0.0)
    soulbound_wallets = soulbound_wallets[["address", "nft_quantity", "balance"]].sort_values("balance", ascending=False)

    if "raw_amount" in recent_transfers.columns:
        recent_transfers = recent_transfers.copy()
        recent_transfers["amount"] = recent_transfers["raw_amount"].apply(int) / (10 ** recent_transfers["decimals"])
    ...
    bundles = {
        "metadata": {
            ...
            "chain": config.CHAIN,
            "updateCadence": {"transactions": "~hourly", "price": "daily"},
        },
        "price": {
            "latest": _df_records(price.tail(1).rename(columns={"price_usd": "priceUsd"}))[0] if not price.empty else None,
            "history": _df_records(price.rename(columns={"price_usd": "priceUsd"})),
            "sourceNote": [
        ...
        "chad": {
            ...
            "summary": {
                "wallets": int(latest_chads["wallet_count"].sum()) if not latest_chads.empty else 0,
                "tibbirHeld": total_chad_balance,
                "avgCoinAgeDays": weighted_age,
            },
            "cohorts": _df_records(chad_cohorts),
            "wallets": _df_records(chad_wallets),
        },
        "soulbound": {
            "summary": _df_records(soulbound_supply.tail(1))[0] if not soulbound_supply.empty else None,
            "history": _df_records(soulbound_supply),
            "wallets": _df_records(soulbound_wallets),
            "holdersUrl": "https://basescan.org/token/0xcabce1fa75aca96b40cc98dd3ab38ba332d9e488",
        },
        "holder-buckets": {
            "buckets": [{"label": label, "pctColumn": pct, "countColumn": count} for pct, count, label in buckets],
            "totalWallets": total_wallets,
            "latest": wallet_vs_supply,
        ...
        "wallet-verification": {
            ...
            "chadWallets": _df_records(chad_wallets),
            "soulboundWallets": _df_records(soulbound_wallets),
        },
        "dataset-details": {
            "contractAddress": config.CONTRACT_ADDRESS,
            "metadata": metadata,
        ...
        "metric-example": _build_coin_age_example_bundle(),
    }

    for name, data in bundles.items():
        print(f"Writing dashboard/{name}.json...")
        _write_json(data, _dashboard_json_path(name))

    return f"Dashboard bundles built: {', '.join(sorted(bundles))}"
